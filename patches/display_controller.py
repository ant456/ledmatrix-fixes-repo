    ...
    * Delegate all actual content to the plugin system — this class contains
    ...
    def __init__(self):
        ...
        try:
            ...
            if on_demand_plugin_id:
                logger.info("On-demand mode detected during initialization: will resume on plugin '%s', but loading all normally-enabled plugins as usual", on_demand_plugin_id)
                # Load every normally-enabled plugin, same as a regular
                # startup would -- NOT just the on-demand plugin. Confirmed
                # on real hardware (2026-09-06) that restricting to only
                # the on-demand plugin here caused every other plugin to
                # become completely unavailable (stuck cycling only that
                # one plugin's own modes, e.g. all 5 Starlark apps and
                # nothing else) any time a restart happened while
                # on-demand mode was still active -- which happens often,
                # since restarts are a normal part of deploying fixes,
                # updates, etc. The original code's intent was presumably
                # a faster startup for a quick on-demand preview, but that
                # assumption breaks down badly for anything longer-lived.
                # The on-demand state itself is still fully restored below
                # so the display correctly resumes on the saved mode --
                # this only changes which plugins get loaded to make that
                # happen, not whether the resume itself works.
                ...
                if on_demand_plugin_id not in discovered_plugins:
                    ...
                    logger.info("On-demand mode: will resume on '%s', all %d enabled plugin(s) still loading normally", on_demand_plugin_id, len(enabled_plugins))
            else:
                enabled_plugins = [p for p in discovered_plugins if self.config.get(p, {}).get('enabled', False)]
    ...
    def _poll_on_demand_requests(self) -> None:
        ...
        try:
            ...
            # memory_ttl=0 bypasses cache_manager's in-memory tier, forcing
            # a fresh read from disk every poll. Without this, the first
            # read this process ever does of this key (likely empty, very
            # early at startup) gets cached in memory for the full max_age
            # (an hour) -- every subsequent poll, every frame, just
            # returned that same stale snapshot instead of ever seeing a
            # different process's (the web UI's) later write. Confirmed on
            # real hardware (2026-09-04): on-demand requests sent without
            # restarting the display service were never picked up at all,
            # while a full restart (fresh process, no stale cache) always
            # worked -- exactly the cross-process-write scenario
            # cache_manager.py's own get() docstring describes this
            # parameter for.
        ...
        if action == 'stop':
            ...
            if self.on_demand_active:
                ...
                self.on_demand_request_id = request_id
            # Clear the request from cache now that it's been handled --
            # without this, memory_ttl=0 (above) means every single frame,
            # forever, re-reads this exact same stop request from disk and
            # re-processes it (confirmed on real hardware, 2026-09-04:
            # thousands of "Received on-demand stop request" log lines
            # firing milliseconds apart, likely degrading render loop
            # performance in addition to spamming the log). Start requests
            # don't need this since they're guarded by request_id/
            # processed_id checks instead, but stop requests are
            # deliberately always-reprocessed by the branch above, so
            # clearing the cache entry is what actually stops the loop.
            self.cache_manager.clear_cache('display_on_demand_request')
            return
    ...
    def _activate_on_demand(self, request: Dict[str, Any]) -> None:
        ...
        # pinned=True restricts on-demand rotation to just the specifically
        # requested mode, rather than every mode belonging to the resolved
        # plugin. This parameter existed but was never actually read
        # anywhere (confirmed 2026-09-06) -- given it real meaning here.
        # Needed for plugins like starlark-apps, where each "mode" is a
        # completely unrelated app (a printer status display vs. an
        # aquarium animation vs. a music now-playing widget, etc.), not
        # related views of the same content the way football-scoreboard's
        # nfl_live/nfl_recent/nfl_upcoming are -- rotating through every
        # other app when the user asked for one specific one isn't the
        # right behavior for that case.
        if pinned:
            ordered_modes = [resolved_mode]
